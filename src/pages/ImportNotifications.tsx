...
import { ParsedTransaction, Category, PAYMENT_METHOD_LABELS, TransactionType } from "@/lib/types";
...
export default function ImportNotifications({ onConfirm, onAddNotification, categoryStore }: Props) {
  ...
  const handleConfirm = () => {
    const txs = parsed.map(p => ({
      ...
      paymentMethod: PAYMENT_METHOD_LABELS[p.method] || p.method, method: p.method,
      recurring: p.isRecurring, recurrenceHint: p.recurrenceHint, aiConfidence: p.aiConfidence,
    }));
    parsed.forEach(p => {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <p className="text-sm text-muted-foreground mb-6">Cole notificações de banco, PIX ou cartão. O Cognit analisa o texto e organiza automaticamente valor, data e categoria.</p>
